...
import numpy as np
import torch as T
from PPO_Learning_SA import Agent
...
if __name__ == '__main__':
    ...
    if plotting:
            ...
            score_history = np.load(os.path.join(output_dir, "score_history_2026-06-21_17-39-49.npy"))
            length_history = np.load(os.path.join(output_dir, "length_history_2026-06-21_17-39-49.npy"))
            value_loss_history = np.load(os.path.join(output_dir, "value_loss_history_2026-06-21_17-39-49.npy"))
            entropy_loss_history = np.load(os.path.join(output_dir, "entropy_loss_history_2026-06-21_17-39-49.npy"))
    ...
    for episode in range(num_test_episodes):
        ...
        obs = agent.scaler.transform(raw_obs)
        episode_reward = 0
        ...
        for step in range(steps_per_episode):
            ...
            state_t = T.tensor(obs, dtype=T.float32).to(agent.actor.device)
            with T.no_grad():
            ...
            obs = agent.scaler.transform(raw_obs_)
