...
import numpy as np
import time
from gym_pybullet_drones.envs.MA_Aviary import MA_Aviary


def _worker(remote, env_kwargs):
    env = None
    try:
        env = MA_Aviary(**env_kwargs)
        while True:
            try:
            ...
            elif cmd == "compute_global_state":
                remote.send((env._computeGlobalState(), None))
...
class Parallel_MA_Aviary():
    ...
    def _computeGlobalState(self):
        if not self.use_mp:
            return self.env._computeGlobalState()

        for remote in self.remotes:
            remote.send(("compute_global_state", None))

        results = [r.recv() for r in self.remotes]

        global_states = np.concatenate([r[0] for r in results], axis=0)
